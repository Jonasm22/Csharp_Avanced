...
decimal GetGroupTicketPriceDiscount(int groupSize, DateTime visitDate)
    => (groupSize, visitDate.DayOfWeek) switch
    {
        ...
        (>= 5 and < 10, _) => 0.12m,
        (>= 10, _  ) => 0.15m,
         (<= 0,_) => throw new ArgumentException("The group size nummer must be positive"),
...
foreach ((var size, var date) in TestDiscountData)
{
    ...
    Console.WriteLine($"discount for a size : {size} - person group on  {date: ddd/MM/d)} is {discount}");
...
{
    (> 0, > 0) => "Upper rigth quadrant",
    (< 0, > 0) => "Upper left quadrant",
    (> 0, < 0) => "Lower rigth quadrant",
    (< 0, < 0) => "Lower rigth quadrant",
